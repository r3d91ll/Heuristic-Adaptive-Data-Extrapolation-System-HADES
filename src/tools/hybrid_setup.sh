#!/bin/bash
#
# HADES Hybrid Environment Setup
# This script sets up a hybrid environment with:
# - Local PostgreSQL (already installed)
# - Docker-based ArangoDB
#
...
# Function to setup ArangoDB with Docker
setup_arango_docker() {
    log "INFO" "Setting up ArangoDB using Docker..."
    
    # Check if ArangoDB container is already running
    if docker ps | grep -q "hades_arangodb"; then
        log "INFO" "ArangoDB container is already running."
    else
        # Check if ArangoDB container exists but is not running
        if docker ps -a | grep -q "hades_arangodb"; then
            log "INFO" "Starting existing ArangoDB container..."
            docker start hades_arangodb
        else
            # Create and start ArangoDB container
            log "INFO" "Creating and starting ArangoDB container..."
            docker run -d \
                --name hades_arangodb \
                -p $ARANGO_PORT:8529 \
                -e ARANGO_ROOT_PASSWORD=$ARANGO_PASSWORD \
                -v hades_arango_data:/var/lib/arangodb3 \
                arangodb:$ARANGO_VERSION
        fi
    fi
    
    # Wait for ArangoDB to start
    log "INFO" "Waiting for ArangoDB to start..."
    sleep 10
    
    # Check if ArangoDB is running
    if ! curl -s http://localhost:$ARANGO_PORT/_api/version > /dev/null; then
        log "ERROR" "ArangoDB is not responding. Please check Docker logs with: docker logs hades_arangodb"
        exit 1
    fi
    
    log "SUCCESS" "ArangoDB container is running."
    
    # Configure ArangoDB
    log "INFO" "Configuring ArangoDB..."
    
    # Create ArangoDB user and database using arangosh in Docker
...
# Generated by hybrid_setup.sh on $(date)
...
run_tests() {
    ...
    # Run the test script
    python3 "$PROJECT_ROOT/src/tools/test_hybrid_connections.py"
...
# Main function
main() {
    echo -e "${BOLD}${BLUE}HADES Hybrid Environment Setup${RESET}"
    echo -e "${BLUE}==============================${RESET}\n"
    
    # Create log file
    touch "$LOG_FILE"
    log "INFO" "Hybrid environment setup started. Log file: $LOG_FILE"
    ...
    log "SUCCESS" "HADES hybrid environment setup completed successfully!"
    log "INFO" "To start using the services, run: source $PROJECT_ROOT/.env"
    log "INFO" "To stop the ArangoDB container, run: docker stop hades_arangodb"
    log "INFO" "To start the ArangoDB container again, run: docker start hades_arangodb"
}
